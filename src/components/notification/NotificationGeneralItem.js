...
class NotificationGeneralItem extends Component {
        ...
        if (count === 1) {
            return (
                <Text style={[styles.usernameText, {color: theme.primaryColor}]}>
                    <Text onPress={()=>navigation.push('Другой профиль', {user: item.users[0]})}>{`${name} `}</Text>
            ...
            return (
                <Text style={[styles.usernameText, {color: theme.primaryColor}]}>
                    <Text onPress={()=>navigation.push('Другой профиль', {user: item.users[0]})}>{`${name}, `}</Text>
                    <Text style={[styles.usernameText, {color: theme.primaryColor}]}>{`${name1}`}</Text>
                    <Text style={[styles.otherText]}>{' и '}</Text>
            ...
            return (
                <Text style={[styles.usernameText, {color: theme.primaryColor}]}>
                    <Text onPress={()=>navigation.push('Другой профилль', {user: item.users[0]})}>{`${name}, `}</Text>
                    <Text style={[styles.usernameText, {color: theme.primaryColor}]}>{`${name1}, `}</Text>
                    <Text style={[styles.usernameText, {color: theme.primaryColor}]}>{`${name2}`}</Text>
                    <Text style={[styles.otherText]}>{' и '}</Text>
    ...
    render() {
        ...
        if (notificationType === 'like') {
            text = `Понравилься ваш пост.`;
            if (count > 1)
                text = `понравился ${count} ваш пост.`
        } else if (notificationType === 'comment') {
            text = 'оставил комментарий под ваш пост.';
        }
            ...
            <View style={styles.container}>
                <View style={styles.userView}>
                    ...
                    <View style={styles.nameView}>
                        ...
                        <Text style={[styles.timeText, {color: theme.secondaryColor}]}></Text>
                    </View>
                    {
